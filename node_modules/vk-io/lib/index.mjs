...
import { Composer, compose, noopNext } from 'middleware-io';
export { Composer } from 'middleware-io';
...
(function (APIErrorCode) {
     ...
     * Code: `148`
     */
    APIErrorCode[APIErrorCode["ACCESS_MENU"] = 148] = "ACCESS_MENU";
    /**
     * Invalid timestamp
     *
     * Code: `150`
 ...
 * Parse attachments
 */
const parseAttachmentRe = /^([a-z]+)(-?\d+)_(\d+)_?(\w+)?$/;
class Attachment {
    /**
     * Constructor
     */
    ...
    async start() {
        ...
        try {
            ...
            const pollingURL = isGroup
                ? server
                : `https://${server}`;
            this.url = new URL$1(pollingURL);
            this.url.search = String(new URLSearchParams$1({
                key,
                act: 'a_check',
                wait: '25',
...
class MessageContext extends Context {
    constructor(options) {
        ...
        if (options.source === UpdateSource.POLLING) {
            this.$filled = false;
            this.applyPayload(transformMessage(options.payload));
        }
        else {
            this.$filled = true;
            this.applyPayload(options.payload);
...
class KeyboardBuilder {
    ...
    payButton({ hash: rawHash }) {
        const hash = typeof rawHash === 'object'
            ? String(new URLSearchParams$1(Object.entries(rawHash)))
            : rawHash;
        return this.addWideButton({
            action: {
                hash,
     ...
     * ```
     */
    callbackButton({ label, payload: rawPayload = {}, color = ButtonColor.SECONDARY }) {
        if (label.length > 40) {
            throw new RangeError('Maximum length of label 40 characters');
        }
        const payload = serializePayload(rawPayload);
...
export { API, APIError, APIErrorCode, APIRequest, APIWorker, Attachment, AttachmentType, Attachmentable, AudioAttachment, AudioMessageAttachment, ButtonColor, CallbackService, CaptchaType, Chain, CollectError, CollectErrorCode, CommentContext, Context, DialogFlagsContext, DialogMessagesContext, DialogNotificationSettingsContext, DocumentAttachment, DonutSubscriptionContext, DonutSubscriptionPriceContext, DonutWithdrawContext, ExecuteError, ExternalAttachment, FriendActivityContext, GiftAttachment, GraffitiAttachment, GroupMemberContext, GroupUpdateContext, GroupUserContext, Keyboard, KeyboardBuilder, LikeContext, LinkAttachment, MarketAlbumAttachment, MarketAttachment, MarketOrderContext, MessageContext, MessageEventContext, MessageFlagsContext, MessageForwardsCollection, MessageSource, MessageSubscriptionContext, MessagesReadContext, NewAttachmentsContext, objects as Objects, ParallelSelectedWorker, ParallelWorker, params as Params, PhotoAttachment, PollAttachment, PollingTransport, ResourceError, ResourceErrorCode, ResourceType, responses as Responses, SequentialWorker, SharedErrorCode, StickerAttachment, StoryAttachment, TypingContext, TypingState, UpdateSource, Updates, UpdatesError, UpdatesErrorCode, Upload, UploadError, UploadErrorCode, VK, VKAppPayloadContext, VKError, VKPayTransactionContext, VideoAttachment, VoteContext, WallAttachment, WallPostContext, WallReplyAttachment, WebhookTransport, applyMixins, createCollectIterator, executes, getRandomId, kSerializeData, parseAttachmentRe, resolveResource, transformAttachments };
